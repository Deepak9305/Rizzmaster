...
const HomePage: React.FC<{ navigate: (path: string) => void }> = ({ navigate }) => …3022 tokens truncated…rketing-container marketing-page-padding">
...
const ARTICLE_INTERNAL_LINKS: Record<string, Array<{ slug: string; label: string }>> = {
  'what-to-text-when-they-stop-replying': [
    ...
    { slug: 'reply-when-she-says-haha', label: 'How to keep a playful chat moving' }
  ],
  'reply-to-dry-texts': [
...
const ArticleResources: React.FC<{ post: BlogPost }> = ({ post }) => {
  if (!post.resources?.length) return null;

  return (
    <section className="rounded-3xl border border-amber-300/15 bg-amber-300/[0.04] p-5 md:p-6">
      <p className="text-xs font-black uppercase tracking-[0.2em] text-amber-200/80">Further reading</p>
      <p className="mt-3 text-sm leading-6 text-white/50">For more perspective on communication, boundaries, and healthy relationship patterns:</p>
      <div className="mt-4 flex flex-col gap-2">
        {post.resources.map((resource) => <a key={resource.url} href={resource.url} target="_blank" rel="noreferrer" className="text-sm font-bold text-amber-100/80 transition-colors hover:text-white">{resource.label} <span aria-hidden="true">-&gt;</span></a>)}
      </div>
    </section>
  );
};
