...
const RizzCard: React.FC<RizzCardProps> = memo(({
  ...
  const handleShare = async () => {
    ...
    await NativeBridge.shareText('Rizz Master', content);
  };
    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="flex gap-1 opacity-60 group-hover:opacity-100 transition-opacity">
          ...
          <button
            onClick={handleShare}
            className="p-1.5 rounded-lg hover:bg-white/10 text-white/70 hover:text-blue-400 transition-colors active:scale-95"
            title="Share"
          >
