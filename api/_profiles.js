...
let hasLoggedMissingPremiumRecompute = false;

const recomputeLegacyPremium = async (client, profile) => {
  const { data, error } = await client.rpc('admin_recompute_premium', {
  ...
  if (error) {
    ...
    const isMissingFunction = error.code === 'PGRST202' || (
      ...
      normalizedMessage.includes('admin_recompute_premium')
    );
    if (isMissingFunction) {
      if (!hasLoggedMissingPremiumRecompute) {
        hasLoggedMissingPremiumRecompute = true;
        console.warn('[Premium] Shared entitlement migration is not applied yet; using the existing Google Play profile state.');
      }
      return profile;
    }

    throw new AppDataError(
      error.message || 'Failed to recompute premium entitlement',
      'PROFILE_BOOTSTRAP_FAILED',
      500
    );
  }
