ALTER TABLE public.dodo_subscriptions
  DROP CONSTRAINT IF EXISTS dodo_subscriptions_status_check;

ALTER TABLE public.dodo_subscriptions
  ADD CONSTRAINT dodo_subscriptions_status_check
  CHECK (status IN ('pending', 'active', 'on_hold', 'paused', 'cancelled', 'failed', 'expired'));

ALTER TABLE public.dodo_subscriptions
  ADD CONSTRAINT dodo_active_subscription_requires_expiry
  CHECK (status <> 'active' OR access_expires_at IS NOT NULL);
...
BEGIN
  ...
  WHERE user_id = p_user_uuid
    AND (
      (status = 'active' AND access_expires_at > now())
  ...
  IF has_google AND has_dodo THEN
    effective_expiry := CASE
      WHEN (NOT has_google_grace AND google_subscription.expires_at IS NULL) THEN NULL
      ELSE GREATEST(
